...
export const studentFormEmailTemplate = (saved: IStudentForm): string => {
  ...
  const personalInfoFields: Array<{ key: keyof IStudentForm; label: string }> = [
  ...
  const renderSkillsSection = (): string => {
    if (!saved.skills || saved.skills.length === 0) {
      return '<div style="color: #666; text-align: center; padding: 20px;">No skills specified</div>';
    }

    const rows: string[][] = [];
    saved.skills.forEach((skill, index) => {
  ...
  const renderEventsSection = (): string => {
    if (!saved.events || saved.events.length === 0) {
      return '<div style="color: #666; text-align: center; padding: 20px;">No events selected</div>';
    }
      ...
      <div style="padding: 40px 32px;">
          ...
          <div style="padding: 24px;">
            ${personalInfoFields
              ...
              <div style="display: flex; padding: 12px 0; border-bottom: 1px solid #eaeaea;">
                <div style="flex: 1; color: #666; font-weight: 500;">${field.label}:</div>
                <div style="flex: 1; color: #1a1a1a; font-weight: 400;">${
                  saved[field.key] || "Not provided"
                }</div>
              </div>
            `
              )
        ...
        <div style="background: #f0f7ff; border-radius: 12px; padding: 0; overflow: hidden; margin: 32px 0;">
          ...
          <div style="padding: 24px; color: #1a1a1a; line-height: 1.6; font-size: 15px;">${
            saved.careerGoal || "Not specified"
          }</div>
        </div>
